### These are functions from process.py or the wsi-dsa-deid that have NOT been modified
## Keeping these in separrate files to make upgrading to new versions easier to track
import re, os
import PIL.Image
...
def redact_topleft_square(image):
    ...
    if short_percentage > 0 and long_percentage > 0:
        ...
        imageDraw.rectangle(
            (0, 0, min(w, h), min(w, h)), fill=background, outline=None, width=0
        )
    return newImage


## SLIGHT MOD -- placed __version__ in here instead of module
...
def redact_format_aperio_add_image(
    key, image, ifds, firstAssociatedIdx, tempdir, aperioValues
):
    """
...
def get_standard_redactions_format_philips(item, tileSource, tiffinfo, title):
    ...
    redactList = {
        "images": {},
        "metadata": {
            "internal;xml;PIIM_DP_SCANNER_OPERATOR_ID": generate_system_redaction_list_entry(
                title
            ),
...
## MODIFIED SLIGHTLY -- added __version__ to the version string
...
def read_ts_as_vips(ts):
    ...
    for tile in ts.tileIterator(tile_size=dict(width=_iterTileSize)):
        _pool_add(
            tasks, (pool.submit(_convert_large_image_tile, tilelock, strips, tile),)
        )
    ...
    if cropAllowed:
        width = max(
            ...
            min(
                ...
                int(
                    math.ceil(
                        max(pt[0] for poly in polygons for loop in poly for pt in loop)
                    )
                ),
            ),
        )
        height = max(
            ...
            min(
                ...
                int(
                    math.ceil(
                        max(pt[1] for poly in polygons for loop in poly for pt in loop)
                    )
                ),
            ),
        )
        ...
        if tifftools.Tag.TileWidth.value in ifd["tags"]:
            for redifd in redifds:
                ...
                if (ifdw == redw and ifdh == redh) or (
                    ifdw == math.ceil(redw / tileWidth) * tileWidth
                    and ifdh == math.ceil(redh / tileHeight) * tileHeight
                ):
                    newifd = redifd
                    break
        if newifd:
...
def get_standard_redactions_format_hamamatsu(item, tileSource, tiffinfo, title):
    ...
    redactList = {
        "images": {},
        "metadata": {
            "internal;openslide;hamamatsu.Reference": generate_system_redaction_list_entry(
                title
            ),
            "internal;openslide;tiff.Software": generate_system_redaction_list_entry(
                get_deid_field(item, metadata.get("openslide", {}).get("tiff.Software"))
...
## MODIFIED -- Removed ImageItem() class
...
#                 "text": (
#                     value
#                     if key != "PIM_DP_UFS_BARCODE"
#                     else base64.b64encode(value.encode()).decode()
#                 ),
#             }
...
#             dictToEtree(xmldict), encoding="utf8", method="xml"
#         ).decode(),
#     }
